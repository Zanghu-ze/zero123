...
def preprocess_image(models, input_im, preprocess):
    ...
    if preprocess:
        ...
        Image.fromarray((input_im * 255).astype(np.uint8)).save('input_im_normal.png')
...
def main_run(raw_im,
    ...
    input_im_np = input_im.cpu().numpy()
    input_im_rearranged = rearrange(input_im_np, 'b c h w -> b h w c')  # 保留批次维度
    input_im_pil = Image.fromarray((input_im_rearranged[0] * 255).astype(np.uint8))  # 取第一个批次
    input_im_pil.save('input_im_boats.png')
...
def predict(device_idx: int =_GPU_INDEX,
            ...
            config: str ="/storage/home/haoze/using_models/zero123/zero123/configs/test-alldirection-fintune-c.yaml",
